/* $Id: lista_estatica.cpp,v 1.1 2009/09/27 22:45:56 raitech Exp raitech $ */
...
Lista::removeEl(int x) {
	 ...
	 */
	if(x >= fim() || x < 1 || vazia())
		/* o g++ nao deixou retornar nada, portanto tenho que retornar o
		 * elemento que o construtor padrao ELem::Elem() cria
		 *
		 * isso gera uma ambiguidade: e se o elemento que foi removido
		 * por exatamente o elemento-padrao? a priori, isso implica em
		 * nao ter checagem de sucesso, um acreditar piamente na lista
		 */
		return (el);
	else {
		el = A[x];
